...
class xfoil():
    ...
    def get_airfoil(self, air_code):
        ...
        self.save_airfoil(f'Naca {air_code}')
        (x, y) = extract.coordinates(f'NACA {air_code} .txt')
        return x, y

    def save_airfoil(self, file_name):
        self.write_xfoil('SAVE')
        self.write_xfoil(f'{file_name}.txt')
        self.close_xfoil()
        (x, y) = extract.coordinates(f'{file_name}.txt')
        return x, y
    ...
    def panel_foil(self, n_panel):
        self.write_xfoil(f'PPAR')
        self.write_xfoil(f'N')
        self.write_xfoil(f'{n_panel}')
        self.write_xfoil(f'')
        self.write_xfoil(f'')
